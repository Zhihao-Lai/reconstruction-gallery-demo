...
function parsePointCloud(buffer) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    colors[p] = view.getUint8(offset + 6) / 255;
    colors[p + 1] = view.getUint8(offset + 7) / 255;
    colors[p + 2] = view.getUint8(offset + 8) / 255;
    offset += 10;
  }
...
async function loadCloud(scene) {
  ...
  detailMeta.innerHTML = `
    <span>${scene.inputs.length}张输入图</span>
    <span>${formatCount(scene.displayPoints)}点</span>
    <span>室外</span>
  `;
  ...
  const response = await fetch(scene.cloud, { cache: "no-store" });
  if (!response.ok) {
    throw new Error(`Failed to load ${scene.cloud}`);
  }
  ...
  statusEl.textContent = `${formatCount(cloud.count)}点`;
}
...
function renderGrid(items) {
  ...
  for (const item of items) {
    ...
    card.innerHTML = `
      ...
      <div class="case-body">
        <h3>${item.title}</h3>
        <p>${formatCount(item.displayPoints)}点 · ${item.inputs.length}张输入图</p>
      </div>
    `;
...
function draw(now) {
  ...
  lastTime = now;
  if (autoSpin && !dragging && pointCount > 0) {
    yaw += dt * 0.16;
  }
...
function onPointerMove(event) {
  ...
  if (dragging && lastPointer) {
    ...
    if (dragMode === "pan") {
      panX = Math.max(-1.5, Math.min(1.5, panX + (dx / Math.max(rect.width, 1)) * 2));
      panY = Math.max(-1.5, Math.min(1.5, panY - (dy / Math.max(rect.height, 1)) * 2));
    } else if (dragMode === "orbit-x") {
      yaw += dx * 0.008;
    } else {
      yaw += dx * 0.008;
...
canvas.addEventListener("pointerdown", (event) => {
  ...
  if (event.ctrlKey || event.metaKey) {
    ...
    dragMode = event.button === 2 || event.button === 1 || event.shiftKey ? "pan" : "rotate";
  }
  autoSpin = false;
  setActiveView(null);
  lastPointer = { x: event.clientX, y: event.clientY };
